...
3. **In-place file mutation stays SCOPED to the protected directories** — `rm`/`rmdir`/`mv`/`cp`, `sed -i`, `tee`, and `>`/`>>` redirection targeting `PROTECTED`. **There is deliberately no global mutation block**; this mirrors the hook's existing posture.

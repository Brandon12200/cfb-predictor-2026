...
**Consequences:** an **80-case allow/deny matrix** (`tests/test_hook_guard_bash.py`) runs the real hook as a subprocess against real payloads — untested hook regex does not merge. Two **stale briefings** in `.claude/agents/code-reviewer.md` were corrected in the same PR: it told the reviewer `reports/` was append-only (**D23 ratified the opposite**), and cited a `data/conferences.py` module that **does not exist**. **Freeze-day dependency:** when `factors/`, `engine/` and the calibration config join `PROTECTED` at the tag, **both** guards inherit them from the one edit — recorded on `docs/FREEZE_CHECKLIST.md`'s hook item and pinned by a test.

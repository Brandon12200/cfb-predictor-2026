...
## D37 — The repository goes public; D3's privacy component superseded — **RATIFIED (owner, 2026-08-08)**
**Date:** 2026-08-08
**Context.** D3 made the repo private with an option to publish in Aug 2026. Two things argue for exercising it now. First, **GitHub does not enforce rulesets on private personal repositories on the free plan** — the `main-protection` ruleset (nine required checks; force-pushes and deletions blocked) is created and Active but **unenforced**. Public visibility turns it on, which matters because the pipeline pushes to `main` unattended all season. Second, and more to the point of the project: the whole thesis is a **publicly verifiable** pre-registration trail. A prediction committed before kickoff is stronger evidence when anyone can check the timestamp. Secondary benefit: unlimited Actions minutes removes any budget pressure on the cadence.
**Decision (owner).** The repository becomes **public**. **D3's privacy component is superseded; the rest of D3 is unchanged** — AI tooling and decision docs remain committed, `includeCoAuthoredBy` stays `false`, and commit messages and PR text carry **no AI attribution**. That constraint was adopted *because* of this moment, and it pays off here: the history is publishable exactly as written, with no scrubbing.
**Pre-flight, evidence not assertion.** A full-history secret scan was run before the flip: **148 commits, 779 text blobs**, enumerated via `git cat-file --batch-all-objects` so unreachable and dangling objects were included, not merely reachable history. Checked for private-key blocks, AWS `AKIA`, GitHub `ghp_/gho_/ghs_`, Slack `xox*`, Google `AIza`, Stripe `sk_/rk_`, `Bearer` literals, and key-shaped assignments of long opaque values. **Result: clean — zero findings.** `.env` was never tracked; only `.env.example`, whose values are all placeholders. **No rotation was required.**
**Accepted consequence.** The authoring email appears in 117 commit headers and becomes publicly scrapeable. **Accepted explicitly by the owner** — the address is already public elsewhere, so the flip discloses nothing new. Rewriting history to a `noreply` address was considered and rejected: it would change every commit SHA, and those SHAs are the audit trail this project exists to produce.
**Consequence for the pipeline.** From this point the ruleset is **live**. The nine required checks gate every PR, so the next PR doubles as their first real enforcement test. Nothing in the workflows assumes a bypass: no rule restricts pushes, so the pipeline's direct commits need none, and the empty bypass list is correct as-built. Verified that the nine required check names match exactly what CI reports (`test`, `verify (0)`…`(5)`, `verify-freeze`, `no-rehearsal-merge`).
...
1. **Tiers 1–2 fire only on a guarantee slot's miss.** A best-effort slot is designed to miss about four
   times in ten (the Saturday slots, at window − 155 min), and the Wed–Fri 17:23 slot — whose lead is
   only 97 min — **nine times in nine**: every one of the nine measured weekday samples (minimum 102

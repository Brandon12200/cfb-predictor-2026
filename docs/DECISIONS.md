...
3. **L2 confirmation architecture (structural, follows SPEC §7.3 + D15):** the engine computes the model-vs-market **base** gap **before** the factors and injects it on the context; a pure `confirm_situational` gate withholds any situational factor whose direction is not confirmed by the base gap (base only, never total — D15 circularity) or an activated physical factor. **This supersedes the 3a "power rating is diagnostic-only / does not touch the edge" property for the confirmation path** — the base gap now legitimately feeds the edge via L2. Rerun determinism is unaffected (pure function of the snapshot).

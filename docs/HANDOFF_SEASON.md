...
| **C13** | addendum §3, PR C | "`resolve_locators` as a make target … the `LINT_PATHS` conflict with #60 is cleared" | **no reachable source.** No file with that name exists in any local or remote branch, in any commit's tree, or in the stash. D42 (a)3 says "delivered separately" and names no location. PR C is blocked until the owner says where the tool comes from. | An omission, not staleness: the tool was never in the repository, so the addendum was already wrong when written. It listed PR C as next in the queue, with its only obstacle cleared, and cited a `LINT_PATHS` conflict with work that is not in the repository. |
| **C14** | addendum §3, D43 candidate | "counts slack against the *next* kickoff window still ahead" | "…the first window still ahead **today**", plus the branch that does warn (`scripts/pipeline_preflight.py:144`, past every window today) | Imprecise rather than wrong, and imprecise when written, since `check_timing` has not changed. The loop reads only `now.weekday()`'s windows. That is why the Sunday 00:58 ET run counted against Sunday's 13:00 window, and why "never warns" would overstate it. |

**Not an error — an orientation update** (D42 (a)1: true when written, stale by audit):

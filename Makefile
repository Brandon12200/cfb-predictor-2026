.PHONY: help install test lint predict grade report verify-phase-0 verify-phase-1 verify-phase-2 verify-phase-3 verify-phase-4 verify-phase-4-5 verify-phase-5 resolve-locators
...
# Three-axis locator check for a document before it merges (D43 (b)): exists / says / reachable.
# Half a tool: claims with no locator (counts, dates, statuses) still need re-deriving (D42 (d)).
resolve-locators:
	@test -n "$(DOC)" || { echo "usage: make resolve-locators DOC=docs/FILE.md [ARGS='--frame SHA']"; exit 2; }
	$(PY) scripts/resolve_locators.py $(DOC) $(ARGS)
